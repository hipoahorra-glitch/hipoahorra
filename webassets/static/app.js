document.addEventListener('DOMContentLoaded', () => {
    ...
    const drawInsuranceChart = () => {
        ...
        const line = (key, color) => {
            context.beginPath(); context.strokeStyle = color; context.lineWidth = 5;
            points.forEach((point, index) => index ? context.lineTo(x(index), y(point[key])) : context.moveTo(x(index), y(point[key])));
            context.stroke();
        };
        line('bank', '#4c7b86'); line('external', '#d97706');
        context.fillStyle = '#475569'; context.font = '24px sans-serif'; context.fillText('30', 40, height - 8); context.fillText('59', width - 35, height - 8);
        context.fillStyle = '#4c7b86'; context.fillText('Banco', 55, 26); context.fillStyle = '#d97706'; context.fillText('Externo', 180, 26);
    };
    drawInsuranceChart(); window.addEventListener('resize', drawInsuranceChart);
